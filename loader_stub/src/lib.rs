...
#[allow(unsafe_op_in_unsafe_fn)]
pub unsafe fn run_loader(payload: &[u8]) {
...
#[unsafe(no_mangle)]
pub extern "system" fn loader_entry(blob_base: *const u8) -> u32 {
    unsafe {
        let size_stub    = *(blob_base as *const u32)       as usize;
        let size_payload = *(blob_base.add(4) as *const u32) as usize;
        let payload_ptr  = blob_base.add(12 + size_stub); //12 = 3 x u32

        let slice = core::slice::from_raw_parts(payload_ptr, size_payload);
        run_loader(slice);
    }
    0 // thread exit code
}
...
fn main() {
    const PAYLOAD: &[u8] = include_bytes!("payload_messagebox.dll");
    unsafe { run_loader(PAYLOAD) };
}
